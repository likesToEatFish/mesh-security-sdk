...
func (s *IntegrationTestSuite) TestIBCWasmHooks() {
	...
	// sender wasm addr
	// senderBech32, err := ibchookskeeper.DeriveIntermediateSender("channel-0", validatorAddr, "mesh")
	...
	s.Eventually(func() bool {
		...
		return sdk.NewInt(amount).Equal(transferAmount) && denom == initialization.MeshIBCDenom && count == 1
	},
		10*time.Second,
		10*time.Millisecond,
	)
