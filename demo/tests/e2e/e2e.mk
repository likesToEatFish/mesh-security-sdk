...
test-e2e-ci:
	@VERSION=$(VERSION) MESH_E2E=True MESH_E2E_DEBUG_LOG=True  go test -mod=readonly -timeout=25m -v $(PACKAGES_E2E)
...
test-e2e-debug: e2e-setup
	@VERSION=$(VERSION) MESH_E2E=True MESH_E2E_DEBUG_LOG=True MESH_E2E_SKIP_CLEANUP=True go test -mod=readonly -timeout=25m -v $(PACKAGES_E2E) -count=1
...
docker-build-debug:
	@DOCKER_BUILDKIT=1 docker build -t mesh:${COMMIT} --platform linux/amd64 --build-arg BASE_IMG_TAG=debug -f ./tests/e2e/e2e.Dockerfile .
	@DOCKER_BUILDKIT=1 docker tag mesh:${COMMIT} mesh:debug
