...
import (
	...
	"github.com/osmosis-labs/mesh-security-sdk/demo/tests/e2e/configurer/chain"
...
func (bc *baseConfigurer) runIBCRelayer(chainConfigA *chain.Config, chainConfigB *chain.Config, hermesContainerName string) error {
	...
	tmpDir, err := os.MkdirTemp("", "mesh-e2e-testnet-hermes-1")
	if err != nil {
		return err
	}
	...
	// create the client, connection and channel between the two mesh chains
