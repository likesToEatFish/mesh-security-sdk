...
// 	err = keepers.StakingKeeper.SetValidatorByConsAddr(pCtx, val)
// 	require.NoError(t, err)
// 	keepers.StakingKeeper.SetValidator(pCtx, val)
// 	_, capturedTombstones := NewMockEvidenceSlashingKeeper()
...
func TestCaptureStakingEvents(t *testing.T) {
	...
	require.NoError(t, err)
	err = keepers.StakingKeeper.SetValidatorByConsAddr(pCtx, valJailed)
	require.NoError(t, err)
