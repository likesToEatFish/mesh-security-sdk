...
import (
	...
	app "github.com/osmosis-labs/mesh-security-sdk/demo/app"
	"github.com/osmosis-labs/mesh-security-sdk/demo/tests/e2e/initialization"
	"github.com/osmosis-labs/mesh-security-sdk/demo/types/assets"
)

func (n *NodeConfig) StoreWasmCode(wasmFile, from string) {
	n.LogActionF("storing wasm code from file %s", wasmFile)
	cmd := []string{"meshd", "tx", "wasm", "store", wasmFile, fmt.Sprintf("--from=%s", from)}
...
func (n *NodeConfig) InstantiateWasmContract(codeID, initMsg, amount, from string) {
	...
	cmd := []string{"meshd", "tx", "wasm", "instantiate", codeID, initMsg, fmt.Sprintf("--from=%s", from), "--no-admin", "--label=ratelimit"}
	if amount != "" {
...
func (n *NodeConfig) Instantiate2WasmContract(codeID, initMsg, salt, amount, fee, from string) {
	...
	cmd := []string{"meshd", "tx", "wasm", "instantiate2", codeID, initMsg, string(encodedSalt), fmt.Sprintf("--from=%s", from), "--no-admin", "--label=ratelimit"}
	if amount != "" {
		cmd = append(cmd, fmt.Sprintf("--amount=%s", amount))
	}
	if fee != "" {
...
func (n *NodeConfig) WasmExecute(contract, execMsg, amount, fee, from string) {
	...
	cmd := []string{"meshd", "tx", "wasm", "execute", contract, execMsg, fmt.Sprintf("--from=%s", from)}
	if amount != "" {
		cmd = append(cmd, fmt.Sprintf("--amount=%s", amount))
	}
	if fee != "" {
...
func (n *NodeConfig) QueryParams(subspace, key string, result any) {
	cmd := []string{"meshd", "query", "params", "subspace", subspace, key, "--output=json"}
...
func (n *NodeConfig) SubmitParamChangeProposal(proposalJSON, from string) {
	...
	cmd := []string{"meshd", "tx", "gov", "submit-proposal", "/mesh/param_change_proposal.json", fmt.Sprintf("--from=%s", from)}
...
func (n *NodeConfig) SubmitAddBurnTaxExemptionAddressProposal(addresses []string, walletName string) int {
	...
	cmd := []string{
		"meshd", "tx", "gov", "submit-legacy-proposal",
...
func (n *NodeConfig) FailIBCTransfer(from, recipient, amount string) {
	...
	cmd := []string{"meshd", "tx", "ibc-transfer", "transfer", "transfer", "channel-0", recipient, amount, fmt.Sprintf("--from=%s", from)}
...
func (n *NodeConfig) SendIBCTransfer(from, recipient, amount, memo string) {
	...
	cmd := []string{"meshd", "tx", "ibc-transfer", "transfer", "transfer", "channel-0", recipient, amount, fmt.Sprintf("--from=%s", from), "--memo", memo}
...
func (n *NodeConfig) SubmitTextProposal(text string, initialDeposit sdk.Coin) {
	...
	cmd := []string{"meshd", "tx", "gov", "submit-proposal", "--type=text", fmt.Sprintf("--title=\"%s\"", text), "--description=\"test text proposal\"", "--from=val", fmt.Sprintf("--deposit=%s", initialDeposit)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainID, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) DepositProposal(proposalNumber int) {
	...
	deposit := sdk.NewCoin(initialization.MeshDenom, sdk.NewInt(20*assets.MicroUnit)).String()
	cmd := []string{"meshd", "tx", "gov", "deposit", fmt.Sprintf("%d", proposalNumber), deposit, "--from=val"}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainID, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) VoteYesProposal(from string, proposalNumber int) {
	...
	cmd := []string{"meshd", "tx", "gov", "vote", fmt.Sprintf("%d", proposalNumber), "yes", fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainID, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) VoteNoProposal(from string, proposalNumber int) {
	...
	cmd := []string{"meshd", "tx", "gov", "vote", fmt.Sprintf("%d", proposalNumber), "no", fmt.Sprintf("--from=%s", from)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainID, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) BankSendWithWallet(amount string, sendAddress string, receiveAddress string, walletName string) {
	...
	cmd := []string{"meshd", "tx", "bank", "send", sendAddress, receiveAddress, amount, fmt.Sprintf("--from=%s", walletName)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainID, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) BankSendFeeGrantWithWallet(amount string, sendAddress string, receiveAddress string, feeGranter string, walletName string) {
	n.LogActionF("bank sending %s from address %s to %s", amount, sendAddress, receiveAddress)
	cmd := []string{"meshd", "tx", "bank", "send", sendAddress, receiveAddress, amount, fmt.Sprintf("--fee-granter=%s", feeGranter), fmt.Sprintf("--from=%s", walletName)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainID, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) BankMultiSend(amount string, split bool, sendAddress string, receiveAddresses ...string) {
	...
	cmd := []string{"meshd", "tx", "bank", "multi-send", sendAddress}
...
func (n *NodeConfig) GrantAddress(granter, gratee string, spendLimit string, walletName string) {
	...
	cmd := []string{"meshd", "tx", "feegrant", "grant", granter, gratee, fmt.Sprintf("--from=%s", walletName), fmt.Sprintf("--spend-limit=%s", spendLimit)}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainID, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) CreateWallet(walletName string) string {
	...
	cmd := []string{"meshd", "keys", "add", walletName, "--keyring-backend=test"}
	outBuf, _, err := n.containerManager.ExecCmd(n.t, n.Name, cmd, "", false)
	require.NoError(n.t, err)
	re := regexp.MustCompile("mesh1(.{38})")
...
func (n *NodeConfig) GetWallet(walletName string) string {
	...
	cmd := []string{"meshd", "keys", "show", walletName, "--keyring-backend=test"}
	outBuf, _, err := n.containerManager.ExecCmd(n.t, n.Name, cmd, "", false)
	require.NoError(n.t, err)
	re := regexp.MustCompile("mesh1(.{38})")
	walletAddr := fmt.Sprintf("%s\n", re.FindString(outBuf.String()))
