...
var (
	...
	txDefaultGasArgs = []string{fmt.Sprintf("--gas=%d", GasLimit), "--fees=0umesh"}
)
...
func NewManager(isDebugLogEnabled bool) (docker *Manager, err error) {
	...
	docker.network, err = docker.pool.CreateNetwork("mesh-testnet")
	if err != nil {
		return nil, err
	}
	return docker, nil
...
func (m *Manager) ExecQueryTxHash(t *testing.T, containerName, txHash string) (bytes.Buffer, bytes.Buffer, error) {
	...
	command := []string{"meshd", "query", "tx", txHash, "-o=json"}
...
func (m *Manager) RunHermesResource(chainAID, meshARelayerNodeName, meshAValMnemonic, chainBID, meshBRelayerNodeName, meshBValMnemonic string, hermesContainerName string, hermesCfgPath string) (*dockertest.Resource, error) {
	hermesResource, err := m.pool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				fmt.Sprintf("MESH_A_E2E_CHAIN_ID=%s", chainAID),
				fmt.Sprintf("MESH_B_E2E_CHAIN_ID=%s", chainBID),
				fmt.Sprintf("MESH_A_E2E_VAL_MNEMONIC=%s", meshAValMnemonic),
				fmt.Sprintf("MESH_B_E2E_VAL_MNEMONIC=%s", meshBValMnemonic),
				fmt.Sprintf("MESH_A_E2E_VAL_HOST=%s", meshARelayerNodeName),
				fmt.Sprintf("MESH_B_E2E_VAL_HOST=%s", meshBRelayerNodeName),
			},
			Entrypoint: []string{
				"sh",
				"-c",
